...
const UserManagement = () => {
    ...
    const [editingUserId, setEditingUserId] = useState<number | null>(null); // NEW: Tracks who we are editing!
    ...
    const handleSaveUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setToastMsg({ 
                text: error.response?.data?.message || 'Failed to save user. Check username availability.', 
                type: 'error' 
            });
        } finally {
    ...
    const handleDeleteUser = async (id: number, name: string) => {
        // Built-in browser confirmation
        if (!window.confirm(`Are you absolutely sure you want to revoke access for ${name}? This action cannot be undone.`)) {
            return;
        }
    ...
    const openEditModal = (user: UserRecord) => {
        ...
        setFormData({
            ...
            password: '', // Leave blank so they don't have to change it!
            role: user.role,
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Joined Date</th>
                ...
                {!isLoading && totalPages > 1 && (
                    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 border rounded-lg text-sm font-bold bg-white hover:bg-gray-50 disabled:opacity-50">Prev</button>
                            <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 border rounded-lg text-sm font-bold bg-white hover:bg-gray-50 disabled:opacity-50">Next</button>
                        </div>
                    </div>
                )}
            </div>

            {/* --- SMART DYNAMIC MODAL (Create & Edit) --- */}
            {isModalOpen && (
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden slide-up">
                        ...
                        <form onSubmit={handleSaveUser} className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="Juan Dela Cruz"/>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Username</label>
                                        <input required type="text" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="jdelacruz"/>
                                    </div>
                                    <div>
                                        {/* Password is only required if we are NOT editing */}
                                        <label className="block text-xs font-bold text-gray-700 mb-1">
                                            {editingUserId ? 'New Password (Optional)' : 'Temporary Password'}
                                        </label>
                                        <input required={!editingUserId} type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder={editingUserId ? "Leave blank to keep current" : "Min 8 characters"}/>
                                    </div>
                                    <div>
                                ...
                                <div className={`space-y-4 transition-opacity duration-300 ${formData.role === 'Student' ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                    <h4 className="font-bold text-orange-500 uppercase text-xs tracking-wider border-b pb-2">Student Profile</h4>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Student ID Number</label>
                                        <input required={formData.role === 'Student'} type="text" value={formData.student_id_number} onChange={e => setFormData({...formData, student_id_number: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="FCU-2026-XXX"/>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Course</label>
                                        <input required={formData.role === 'Student'} type="text" value={formData.course} onChange={e => setFormData({...formData, course: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="BS Information Technology"/>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Year Level</label>
                                        <input required={formData.role === 'Student'} type="number" min="1" max="5" value={formData.year_level} onChange={e => setFormData({...formData, year_level: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="e.g. 3"/>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Assigned Department</label>
                                        <input required={formData.role === 'Student'} type="text" value={formData.assigned_office} onChange={e => setFormData({...formData, assigned_office: e.target.value})} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="College of Computer Studies"/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 flex justify-end gap-3 pt-4 border-t border-gray-100">
                                ...
                                <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm disabled:bg-blue-400 flex items-center gap-2">
