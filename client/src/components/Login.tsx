...
type LoginProps = {
    // 1. Update this to accept role and name
...
const Login = ({ onLoggedIn }: LoginProps) => {
    ...
    // 2. Here is the handleLogin function!
    const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (token) {
                // Store token, role, and name securely in local storage
                localStorage.setItem('auth_token', token);
                localStorage.setItem('user_role', response.data.user.role);
                localStorage.setItem('user_name', response.data.user.fullname);
            }

            // Pass the role and name up to App.tsx
            onLoggedIn?.(response.data.user.role, response.data.user.fullname);
